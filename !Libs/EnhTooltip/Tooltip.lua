...
function clearTooltip()
	...
	for ttText in getglobalIterator("EnhancedTooltipText%d") do
		...
		ttText:SetFont(STANDARD_TEXT_FONT, 15);
	end
...
function getTextGSC(money, exact)
	...
	local GSC = ""
	if (g > 0) then
		GSC = GSC..g.."|cffffd700金|r"
	end
	if (s > 0) then
		GSC = GSC .. s .. "|cffe6e6e6银|R"
	end
	if (c > 0) then
		GSC = GSC .. c .. "|cffeda55f铜|R"
	end
	if (GSC == "") then
		GSC = GSC_NONE
	end
...
function lineSize_Large()
	...
	if (curLine == 0) then return end
	local line = getglobal("EnhancedTooltipText"..curLine)
	line:SetFont(STANDARD_TEXT_FONT, 15)
end
